...
def process_local_file(self) -> tuple[bool, requests.Response]:
    if hasattr(self, "_KrixikBasePipeline__presigned_post_url_results"):
        if self._KrixikBasePipeline__presigned_post_url_results is None:
            raise ValueError(
                "presigned_post_url_results is None - cannot upload file for processing"
            )
        ...
        raise ValueError(
            "presigned_post_url_results is not in self - cannot upload file for processing"
        )
    ...
    try:
        if self.local_file_path is None:
            raise ValueError(
                "local_file_path is None - cannot upload file for processing"
            )
        ...
        raise requests.exceptions.HTTPError(
            f"local file upload request with upload request_id {self.process_id} failed with HTTPError {e}"
        )
        ...
        raise requests.exceptions.Timeout(
            f"local file upload request with upload request_id {self.process_id} failed with Timeout {e}"
        )
        ...
        raise Exception(
            f"local file upload request with upload request_id {self.process_id} failed with exception {e}"
        )
    finally:
...
def get_presigned_url(self, payload_data):
    try:
        ...
        raise requests.exceptions.ConnectionError(
            f"request failed with ConnectionError {e}"
        )
    except requests.exceptions.Timeout as e:
        raise requests.exceptions.Timeout(f"request failed with Timeout {e}")
    except requests.exceptions.RequestException as e:
        raise requests.exceptions.RequestException(
            f"request failed with request exception {e}"
        )
    except Exception as e:
