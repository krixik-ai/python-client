...
def expire_time_checker(expire_time: str) -> None:
    ...
    if expire_time < EXPIRE_TIME_MIN:
        raise ValueError(
            f"expire_time {expire_time} must be at least {EXPIRE_TIME_MIN} seconds in the future"
        )
    if expire_time > EXPIRE_TIME_MAX:
        raise ValueError(
            f"expire_time {expire_time} must be at most {EXPIRE_TIME_MAX} seconds in the future"
        )
