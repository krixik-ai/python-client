...
def is_valid_timestamp(timestamp: str) -> None:
    for format_str in timestamp_valid_formats:
        try:
            ...
            raise ValueError(
                f"{timestamp} must be a valid date but was given as {timestamp}"
            )
...
def numerical_timestamp_checker(timestamp_int: int, timestamp_name: str) -> None:
    if not isinstance(timestamp_int, int):
        raise TypeError(
            f"{timestamp_name} must be an integer but was given as {timestamp_int}"
        )
    if timestamp_int < 0:
        raise ValueError(
            f"{timestamp_name} must be a positive integer but was given as {timestamp_int}"
        )
...
def string_timestamp_to_int(timestamp_str: str, timestamp_name: str) -> int | None:
    ...
    if dt_check == 0:
        raise ValueError(
            f"{timestamp_name} must be in format -- YYYY-MM-DD -- or -- YYYY-MM-DD HH:MM:SS -- but was given as {timestamp_str}"
        )
...
def is_stump(
    timestamp_str: str, start_end: str | None = None, verbose: bool = False
) -> str:
...
def is_after_bot(timestamp: str, timestamp_name: str) -> None:
    ...
    if timestamp < "2023-01-01":
        raise ValueError(
            f"{timestamp_name} must be after 2023-01-01 but was given as {timestamp}"
        )

    # check if timestamp occurs after the year 9999
    if timestamp > "9999-12-31":
        raise ValueError(
            f"{timestamp_name} must be before 9999-12-31 but was given as {timestamp}"
        )


def timestamp_checker(
    timestamp: str, timestamp_name: str, start_end: str, verbose: bool = False
) -> int | None:
    # check if timestamp is a string
    if not isinstance(timestamp, str):
        raise TypeError(
            f"{timestamp_name} must be a string but was given as {timestamp}"
        )
...
def timestamp_bookends_checker(
    ...
    if created_at_start is not None:
        created_at_start_int = timestamp_checker(
            created_at_start, "created_at_start", start_end="start", verbose=verbose
        )

    if created_at_end is not None:
        created_at_end_int = timestamp_checker(
            created_at_end, "created_at_end", start_end="end", verbose=verbose
        )

    if last_updated_start is not None:
        last_updated_start_int = timestamp_checker(
            last_updated_start, "last_updated_start", start_end="start", verbose=verbose
        )

    if last_updated_end is not None:
        last_updated_end_int = timestamp_checker(
            last_updated_end, "last_updated_end", start_end="end", verbose=verbose
        )
    ...
    if created_at_start_int is not None and created_at_end_int is not None:
        if created_at_start_int > created_at_end_int:
            raise ValueError(
                f"created_at_start {og_created_at_start} must occur before created_at_end {og_created_at_end}."
            )

    if last_updated_start_int is not None and last_updated_end_int is not None:
        if last_updated_start_int > last_updated_end_int:
            raise ValueError(
                f"last_updated_start {og_last_updated_start} must occur before last_updated_end {og_last_updated_end}."
            )
...
def convert_timestamps(
    ...
    if created_at_start is not None:
        ...
        created_at_start_int = string_timestamp_to_int(
            created_at_start, "created_at_start"
        )
    if last_updated_start is not None:
        last_updated_start = is_stump(last_updated_start, "start", verbose=False)
        last_updated_start_int = string_timestamp_to_int(
    ...
    if last_updated_end is not None:
        ...
        last_updated_end_int = string_timestamp_to_int(
