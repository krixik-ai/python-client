...
def local_file_path_checker(local_file_path: str) -> None:
    ...
    if "." + local_file_path.split(".")[-1] not in valid_extensions:
        raise ValueError(
            f"invalid local_file_path: please check that it ends with an allowed extension which include these: {valid_extensions} - your input local_file_path is {local_file_path}"
        )
