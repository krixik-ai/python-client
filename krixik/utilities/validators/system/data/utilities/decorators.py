...
def type_check_inputs(func):
    ...
    def wrapper(*args, **kwargs):
        try:
            ...
            file_names_module = importlib.import_module(
                f"krixik.utilities.validators.system.data.{module_input_type}.file_names"
            )
            local_file_path_module = importlib.import_module(
                f"krixik.utilities.validators.system.data.{module_input_type}.local_file_path"
            )
            symbolic_file_paths_module = importlib.import_module(
                f"krixik.utilities.validators.system.data.{module_input_type}.symbolic_file_paths"
            )

            file_names_checker = file_names_module.file_names_checker
            individual_file_name_checker = (
                file_names_module.individual_file_name_checker
            )
            local_file_path_checker = local_file_path_module.local_file_path_checker

            symbolic_file_paths_checker = (
                symbolic_file_paths_module.symbolic_file_paths_checker
            )
            individual_symbolic_file_path_checker = (
                symbolic_file_paths_module.individual_symbolic_file_path_checker
            )
            ...
            if "local_file_path" in list(kwargs.keys()):
                ...
                path_to_first_module = (
                    f"{library_base_dir}/modules/{first_module}/module.yml"
                )
                default_module_config = get_module_available_defaults(
                    path_to_first_module
                )
                ...
                if "." + local_file_path.split(".")[-1] not in allowable_data_types:
                    raise ValueError(
                        f"the file extension '{local_file_path.split('.')[-1]}' is not allowed for this module"
                    )
            ...
            if "file_name" in list(kwargs.keys()) and "local_file_path" in list(
                kwargs.keys()
            ):
