...
def get_module_details(module_name: str) -> dict:
    if not isinstance(module_name, str):
        raise Exception(
            f"module_name must be a string, your input {module_name} is of type {type(module_name)}"
        )

    if module_name not in available_modules:
        raise Exception(
            f"user defined module {module_name} does not exist, currently available modules: {available_modules}"
        )
    ...
    if input_data_structure is not None:
        input_data_example = (
            input_data_structure().data_example
            if hasattr(input_data_structure, "data_example")
            else None
        )
    ...
    if output_data_structure is not None:
        output_data_example = (
            output_data_structure().data_example
            if hasattr(output_data_structure, "data_example")
            else None
        )
