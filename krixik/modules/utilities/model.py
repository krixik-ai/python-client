...
def model_validator(model: dict, available_models: list) -> None:
    if not isinstance(model, str):
        raise ValueError(
            f"model: must be a string - the model entered is not in this required form - {model}"
        )

    if model not in available_models:
        raise ValueError(
            f"model: must be one of {available_models} - the model entered is not in this required form - {model}"
        )


def model_standardizer(
    model_selection: dict, default_model: str, default_params: dict
) -> dict:
...
def model_selection_validator(
    model_selection: dict, available_models: list, params_validator: Callable
) -> None:
